import { deleteProjectAction } from '@/app/action';
...
import { allJobExperianceFromDB, allProjectFromDB } from '@/database/queries';
      ...
      <ul className="space-y-2">
        {projects.map((project , i) => (
          <li key={i} className="border-b pb-2 py-4 flex gap-4 items-center">
            ...
            <DeleteButton id={project?._id?.toString() as string} deleteAction={deleteProjectAction}/>
          </li>
        ))}
      </ul>
    </div>
      ...
      <ul className="space-y-2">
        {experiances.map((experiance , i) => (
          <li key={i} className="border-b pb-2 py-4 flex gap-4 items-center">
            ...
            <h3 className="font-semibold bg-gray-300 text-black px-4 py-2">{experiance.company}</h3>
            
            <DeleteButton id={experiance?._id?.toString() as string} deleteAction={deleteProjectAction}/>
          </li>
        ))}
      </ul>
    </div>
